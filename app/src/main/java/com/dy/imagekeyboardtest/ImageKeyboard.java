package com.dy.imagekeyboardtest;
...
public class ImageKeyboard extends InputMethodService {
    ...
    private static final String AUTHORITY = "com.dy.imagekeyboardtest.inputcontent";
    ...
    private void doCommitContent(@NonNull String description, @NonNull String mimeType,
                                 @NonNull File file) {
        Log.d(TAG, "doCommitContent file : " + file.toString());
    ...
    public View onCreateInputView() {
        ...
        if (!imagesDir.exists() && !imagesDir.isDirectory()) {
            if (!imagesDir.mkdir()) {
                Log.d(TAG, "imagesDir Directory creation failed.");
            }
        }
        ...
        for (int i = 0; i < rawFiles.length; i++) {
            ...
            Log.d(TAG, "ImgButton setTag: " + rawFiles[i]);
            ImgButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    String emojiName = view.getTag().toString().replaceAll("_", "-");
                    Log.d(TAG, "onClick - info : " + view.getTag().toString() + ".png " + getResources().getIdentifier(view.getTag().toString(), "raw", getPackageName()));
//                    final File file = getFileForResource(ImageKeyboard.this, getResources().getIdentifier(view.getTag().toString(), "raw", getPackageName()), imagesDir, "${view.getTag().toString()}.png");
                    final File file = getFileForResource(ImageKeyboard.this, getResources().getIdentifier(view.getTag().toString(), "raw", getPackageName()), imagesDir, view.getTag().toString() + "png");
                    Log.d(TAG, "file info : " + file.toString());
